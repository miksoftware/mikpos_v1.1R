<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Colores</h1>
            <p class="text-slate-500 mt-1">Gestiona los colores de productos</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
        <div class="relative">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" class="block w-full pl-10 pr-3 py-2.5 border border-slate-200 rounded-xl bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-[#ff7261]/50 focus:border-[#ff7261] transition-all sm:text-sm" placeholder="Buscar color...">
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center text-sm font-semibold text-slate-500 uppercase">Código HEX</th>
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4"><div class="w-8 h-8 rounded-lg border border-slate-200 shadow-sm" style="background-color: {{ $item->hex_code ?? '#ccc' }}"></div></td>
                        <td class="px-6 py-4 font-medium text-slate-900">{{ $item->name }}</td>
                        <td class="px-6 py-4 text-center"><span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-mono bg-slate-100 text-slate-700">{{ $item->hex_code ?? '-' }}</span></td>
                    ...
                    @empty
                    <tr><td colspan="5" class="px-6 py-12 text-center text-slate-500">No hay colores registrados</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="relative z-[100]"><div class="fixed inset-0 bg-slate-900/75 backdrop-blur-sm z-[100]" wire:click="$set('isModalOpen', false)"></div>
        <div class="fixed inset-0 z-[101] overflow-y-auto"><div class="flex min-h-full items-center justify-center p-4">
            <div class="relative w-full max-w-md bg-white rounded-2xl shadow-xl">
                <div class="px-6 py-4 border-b border-slate-200"><h3 class="text-lg font-bold text-slate-900">{{ $itemId ? 'Editar' : 'Nuevo' }} Color</h3></div>
                <div class="px-6 py-4 space-y-4">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Nombre *</label><input wire:model="name" type="text" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#ff7261]/50 focus:border-[#ff7261]" placeholder="Ej: Rojo, Azul Marino">@error('name')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror</div>
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Código HEX</label>
                        <div class="flex gap-2">
                            <input wire:model="hex_code" type="color" class="w-12 h-10 p-1 border border-slate-300 rounded-lg cursor-pointer">
                            <input wire:model="hex_code" type="text" class="flex-1 px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#ff7261]/50 focus:border-[#ff7261] font-mono" placeholder="#000000">
                        </div>
    ...
    <div class="relative z-[100]"><div class="fixed inset-0 bg-slate-900/75 backdrop-blur-sm z-[100]" wire:click="$set('isDeleteModalOpen', false)"></div>
        <div class="fixed inset-0 z-[101] overflow-y-auto"><div class="flex min-h-full items-center justify-center p-4">
            <div class="relative w-full max-w-md bg-white rounded-2xl shadow-xl p-6 text-center">
                ...
                <h3 class="text-lg font-bold text-slate-900 mb-2">Eliminar Color</h3>
