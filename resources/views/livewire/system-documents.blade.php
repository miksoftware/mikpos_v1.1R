<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Documentos Sistema</h1>
            <p class="text-slate-500 mt-1">Configura los tipos de documentos para movimientos de inventario</p>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <div>
                                ...
                                @if($item->description)
                                <p class="text-xs text-slate-500 mt-0.5">{{ Str::limit($item->description, 50) }}</p>
                                @endif
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-sm font-bold bg-gradient-to-r from-[#ff7261]/10 to-[#a855f7]/10 text-[#a855f7]">{{ $item->prefix }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="font-mono text-sm text-slate-600">{{ $item->getCurrentNumber() }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
            ...
            <div class="relative w-full max-w-md bg-white rounded-2xl shadow-xl">
                ...
                <div class="px-6 py-4 space-y-4">
                    ...
                    <div>
                        ...
                        <input wire:model="next_number" type="number" min="1" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#ff7261]/50 focus:border-[#ff7261]">
                        @error('next_number')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        @if($prefix)
                        <p class="text-xs text-slate-500 mt-1">Vista previa: <span class="font-mono font-medium">{{ strtoupper($prefix) }}-{{ str_pad($next_number, 5, '0', STR_PAD_LEFT) }}</span></p>
                        @endif
                    </div>
                    <div>
