...
### Overview
Ingredient management for restaurant/food operations. Ingredients are the raw materials used to compose products (composite products). They have their own stock tracking and can optionally be sold directly.

### Two-Tab Interface
The Ingredients component (`/ingredients`) has two tabs:
1. **Ingredientes** - Individual ingredient management
2. **Grupos** - Ingredient group management (interchangeable options)

### Ingredient Model Fields
- `branch_id`: Branch ownership (branch-filtered)
- `sku`: Auto-generated (`ING-XXXXX`)
- `name`, `description`: Basic info
- `category_id`: Optional category
- `unit_id`: Required unit of measurement
- `tax_id`: Optional tax rate
- `cost`: Purchase cost
- `sale_price`: Sale price (if available for sale)
- `price_includes_tax`: Boolean
- `available_for_sale`: Boolean - if true, ingredient can be sold directly in POS
- `current_stock`, `min_stock`, `max_stock`: Stock tracking (DECIMAL 12,3)
- `image`: Optional image
- `is_active`: Status

### Ingredient Groups (Grupos de Ingredientes)
Groups represent interchangeable ingredient options for a composite product. Example: "Tipo de Proteína" group with options: Pollo, Res, Cerdo.

- `IngredientGroup` model: `branch_id`, `name`, `description`, `is_active`
- `IngredientGroupOption` model: `ingredient_group_id`, `ingredient_id`, `quantity`, `sort_order`
- A group must have at least 2 options
- No duplicate ingredients within the same group
- Options are ordered by `sort_order`

### Ingredient Price Calculation
```php
// Price without tax (when price_includes_tax = true)
$priceWithoutTax = $sale_price / (1 + ($tax->value / 100));

// Price with tax (when price_includes_tax = false)
$priceWithTax = $sale_price * (1 + ($tax->value / 100));
```

### Low Stock Detection
```php
$ingredient->isLowStock(); // returns true when current_stock <= min_stock
```

### Permissions
- `ingredients.view/create/edit/delete`
- Same permissions cover both ingredients and groups

### Deletion Protection
- Cannot delete an ingredient that is assigned to any group (must deactivate instead)

### Database Tables
- `ingredients` - Main ingredient records
- `ingredient_groups` - Group definitions
- `ingredient_group_options` - Group ↔ ingredient pivot with quantity and sort_order
  - Unique constraint: `(ingredient_group_id, ingredient_id)`


## Composite Products (Productos Compuestos)

### Overview
Products can be either `independent` (standard product) or `composite` (assembled from ingredients). This is controlled by the `product_type` field on the `products` table.

### Product Types
- `independent` (default): Standard product, sold as-is, no ingredient links
- `composite`: Product assembled from fixed ingredients and/or ingredient groups

### Composite Product Structure
A composite product can have:
1. **Fixed Ingredients** (`product_ingredients` pivot): Ingredients always included with a specific quantity
2. **Ingredient Groups** (`product_ingredient_groups` pivot): Groups where the customer chooses one option

### Database Tables
- `products.product_type` - `'independent'` or `'composite'` (VARCHAR 20, default 'independent')
- `product_ingredients` - Pivot: `product_id`, `ingredient_id`, `quantity` (DECIMAL 12,3), `sort_order`
  - Unique constraint: `(product_id, ingredient_id)`
- `product_ingredient_groups` - Pivot: `product_id`, `ingredient_group_id`, `sort_order`
  - Unique constraint: `(product_id, ingredient_group_id)`

### Product Model Relationships
```php
// Fixed ingredients (HasMany to pivot model)
$product->productIngredients()  // HasMany ProductIngredient
$product->ingredients()         // BelongsToMany Ingredient (via product_ingredients)

// Ingredient groups (HasMany to pivot model)
$product->productIngredientGroups()  // HasMany ProductIngredientGroup
$product->ingredientGroups()         // BelongsToMany IngredientGroup (via product_ingredient_groups)

// Check type
$product->isComposite()  // returns true if product_type === 'composite'
```

### UI in Products Component
- Product creation/edit form shows type selector: "Independiente" vs "Compuesto"
- Composite products show a purple "Compuesto" badge in the product list
- When composite is selected, an ingredients section appears to add fixed ingredients and groups
- Switching from composite to independent removes all ingredient/group links

### Ingredient Sync on Save
When saving a composite product:
```php
if ($product_type === 'composite') {
    $this->syncProductIngredients($item);
} else {
    // Remove all links if switching to independent
...
## Zones & Tables Module (Zonas y Mesas)

### Overview
Restaurant zone and table management for organizing physical dining areas. Each zone belongs to a branch and contains multiple tables.

### Two-Tab Interface
The ZonesAndTables component (`/zones-tables`) has two tabs:
1. **Zonas** - Zone management (dining areas)
2. **Mesas** - Table management within zones

### Zone Model Fields
- `branch_id`: Branch ownership
- `name`: Zone name (e.g., "Terraza", "Salón Principal", "Barra")
- `description`: Optional description
- `color`: HEX color code (default `#6366f1`) for visual identification
- `is_active`: Status

### RestaurantTable Model Fields
- `zone_id`: Parent zone (cascading delete)
- `name`: Table name/number (e.g., "Mesa 1", "Barra 3")
- `capacity`: Seating capacity (integer, 1-50, default 4)
- `status`: `'available'` | `'occupied'` | `'reserved'` (default 'available')
- `is_active`: Status

**Important**: The model uses `protected $table = 'restaurant_tables'` since `tables` is a reserved word.
...
### Computed Attributes
```php
$table->status_label  // "Disponible", "Ocupada", "Reservada"
$table->status_color  // "green", "red", "yellow"
$table->isAvailable() // true if status === 'available' && is_active
```

### Deletion Rules
- Cannot delete a zone that has tables (must delete tables first)
- Tables can be deleted freely

### Permissions
- `zones_tables.view/create/edit/delete`

### Database Tables
- `zones` - Zone definitions per branch
- `restaurant_tables` - Tables within zones


## Reception Module (Recepción)

### Overview
Visual dashboard showing real-time table status across zones. Designed for restaurant hosts/hostesses to manage seating.

### Route
- `/reception` - No specific permission middleware (just `auth`)
- Appears as top-level menu item after Dashboard

### Features
- Branch selector for super_admin users
- Zone tabs with color indicators
- Global stats: total tables, available, occupied, reserved
- Per-zone stats: available/occupied/reserved counts
- Visual table cards with status colors and capacity info
- Color-coded status indicators matching zone colors

### Zone Navigation
- Zones displayed as horizontal tabs
- First zone auto-selected on load
- Each tab shows zone name and table count
- Active zone highlighted with its configured color

### Table Card Display
Each table card shows:
- Table name
- Capacity (person icon + number)
- Current status with color indicator
- Zone color accent on the card border/header
